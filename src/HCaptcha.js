...
module.exports = class HCaptchaSolver{
    ...
    async solve(rqdata){
        ...
        let solvedCaptcha = await this._postCaptcha(captcha,captcha_solution)
    ...
    async _solveCaptcha(captcha){
        ...
        if(!postImages.url) return {status:0,message:"Unable To Submit Images For Recognition"}
        this.usedSolves++
    ...
    async _postCaptcha(captcha,answers){
        ...
        if(submittedCaptcha.generated_pass_UUID) return {status:1,key:submittedCaptcha.generated_pass_UUID}
